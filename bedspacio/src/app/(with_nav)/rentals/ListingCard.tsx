        ...
        <Link key={detail.room_uuid} href={`/rentals/${detail.room_uuid}`} className="group flex flex-col items-center h-[500px] bg-[#FAFAFA] rounded-[10px] p-3 gap-2 border-1 border-[#1D242B]/25 cursor-pointer hover:border-[#1D242B] active:bg-[#C7EEFF] transition-all duration-100">
            <div className="relative flex items-center justify-center w-full min-h-[300px] bg-[#C7EEFF] rounded-[5px]">
                <div className="absolute top-2 left-2 flex items-center px-2 py-1 rounded-[5px] bg-[#1D242B] gap-1 z-10">
                    ...
                    <span className="text-[12px] text-[#FAFAFA]">{detail.gender !== 'male' ? 'Female' : 'Male'}</span>
                    <span className="text-[12px] text-[#FAFAFA]">|</span>
                    <span className={`text-[12px] ${detail.slot ? 'text-[#77D970]' : 'text-[#FF0075]'}`}>{detail.slot > 0 ? "Available" : "Unavailable"}</span>
                </div>
            ...
            <div className="flex flex-col items-start justify-between h-full w-full gap-1">
                ...
                <div className="flex flex-col gap-2 w-full">
                    <div className="flex flex-col items-start">
                        ...
                        <span className="text-[#1D242B] text-[16px] pt-2">Branch: <strong>{detail.branch_name}</strong></span>
                    </div>

                    <div className="flex items-start justify-start w-full gap-1">
                        {detail.inclusions.slice(0, 3).map((inc) => (
                            <span key={inc.id} className="bg-[#0077C0] rounded-full text-[#FAFAFA] text-[12px] px-2 min-w-[50px] truncate">{inc.inclusion}</span> 
                        ))}
