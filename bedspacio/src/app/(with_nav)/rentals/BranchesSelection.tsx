...
export default function BranchSelection ({ branchData, selectedBranch, setSelectedBranch }: BranchProps) {
    ...
    const selectedBranchName =
        selectedBranch === "All"    
            ? "All"
            : branchData.find(b => b.id.toString() === selectedBranch)?.name;
                ...
                <div className='absolute top-21 z-20 flex flex-col items-start justify-start w-full rounded-[5px] bg-[#FAFAFA] overflow-hidden'>
                    <div className='flex flex-col w-full'>
                        {branchData.map((branch, index) => (
                            <span key={branch.id} onClick={() => {setSelectedBranch(branch.id.toString()); setOpenBranch(false) }} className='p-3 text-[#0077C0] text-[16px] font-bold cursor-pointer hover:bg-[#C7EEFF] active:bg-[#C7EEFF] w-full'>{branch.name}</span>
                        ))}
                        <span onClick={() => {setSelectedBranch("All"); setOpenBranch(false) }} className='p-3 text-[#0077C0] text-[16px] font-bold cursor-pointer hover:bg-[#C7EEFF] active:bg-[#C7EEFF] w-full'>{'All'}</span>
