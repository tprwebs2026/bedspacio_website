...
// import Close from '@/asset/icon/log-out.svg'
...
//                             const Icon = stageIcons[inq.ghl_status as keyof typeof stageIcons];
//                             return (
...
//                                         {Icon && <Icon className="w-[25px] h-[25px] opacity-75"/> }
//                                         <span className="text-[14px] font-bold text-[#1D242B] opacity-80">{inq.ghl_status.charAt(0).toUpperCase() + inq.ghl_status.slice(1).toLowerCase()}</span>
//                                     </div>
...
// ------------------- FALLBACK IF GHL IS NOT INTEGRATED ------------------- //
        ...
        <div className="flex w-full h-full overflow-y-auto">
            ...
            <div className="flex flex-col w-full h-auto p-[1rem] xl:px-[8rem] lg:px-[1rem] gap-[1rem]">
                ...
                <div className='flex flex-col w-full rounded-[5px] gap-2'>
                    ...
                    <div className="grid grid-cols-5 w-full rounded-[5px] gap-2">
                        {inquiries.map((inq, index) => {
                            const Icon = stageIcons[inq.inq_status as keyof typeof stageIcons];
                            return (
                                <Link key={index} href={`/admin/inquiry?inq_status=${inq.inq_status}`} className="flex flex-col w-full gap-2 items-start justify-evenly p-4 rounded-[10px] cursor-pointer border border-[#1D242B]/50 hover:bg-[#0077C0]/10 hover:-translate-y-1 transition-all duration-100">
                                    <div className="flex items-center justify-start gap-2 w-full">
                                        {Icon && <Icon className="w-[25px] h-[25px] opacity-75" />}
                                        <span className="text-[14px] font-bold text-[#1D242B] opacity-80">{inq.inq_status}</span>
                                    </div>
                ...
                <div className='grid grid-cols-[3fr_2fr] w-full gap-[1rem] pb-[1rem]'>
                    ...
                    <div className='flex flex-col border border-[#1D242B]/50 p-4 rounded-[10px] gap-[1rem]'>
                        ...
                        <div className='flex flex-col items-center w-full h-full gap-1'>
                            <div className='grid grid-cols-4 justify-items-center place-items-center w-full text-[12px] text-[#1D242B]/50 font-bold pb-1'>
                                <span>Reference #</span>
                                <span>Name</span>
                                <span>Stage</span>
                                <span>Submmitted at</span>
                            ...
                            <div className='flex flex-col w-full h-[300px] overflow-y-auto thin-scrollbar gap-1'>
                                {recentInquiries.length > 0 ? (
                                    <>
                                        {recentInquiries.map(recent => (
                                            <Link href={`/admin/inquiry?page=1&search=${recent.reference_number}`} key={recent.id} className='group grid grid-cols-4 place-items-center justify-items-center w-full text-[14px] text-[#1D242B] border border-[#1D242B]/20 py-2 font-bold rounded-[5px] bg-[#C7EEFF]/50 hover:bg-[#0077C0]/25 active:bg-[#C7EEFF]/50'>
                                                <span>{recent.reference_number}</span>
                                                <span>{recent.fullname}</span>
                                                {/* <span>{formatDate(recent.target_move_in)}</span> */}
                                                <span>{recent.inq_status}</span>
                                                <span className='flex items-center gap-[1rem]'>
                                                    {timeAgo(recent.created_at)}
                                                    <ArrowRight className="opacity-0 group-hover:opacity-100 group-hover:-translate-x-1 transition-all duration-100 w-[20px] h-[20px] stroke-[#1D242B] stroke-2" />
                                                </span>
