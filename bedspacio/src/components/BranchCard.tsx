...
export default async function BranchCard () {
    ...
    return (
        <div className="flex items-center gap-[1rem]flex flex-col xl:flex-row lg:flex-row md:flex-row items-center justify-center gap-[1rem] p-3 w-full">
            {branches.map(branch => (
                <Link key={branch.id} href={""} className='relative flex items-end justify-start w-full h-[200px] xl:h-[337.5px] lg:h-[337.5px] bg-[#1D242B] rounded-[10px] transition-transform duration-200 hover:-translate-y-1 active:translate-y-1 overflow-hidden'>

                    <img src={`data:image/webp;base64,${branch.branch_image}`} alt="" className="w-full h-full object-cover absolute inset-0"/>
