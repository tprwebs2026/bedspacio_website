...
export default async function BranchRoomCard () {
    ...
    const branches = await getBranchPreview();
    const bedspaceImage = await getBedspaceTypeImage();
    const apartmentImage = await getApartmentTypeImage();
    ...
    return (
        <section className='flex flex-col w-full min-h-[800px] bg-[#FAFAFA] items-center justify-start px-[1rem] xl:px-[8rem] lg:px-[4rem] md:px-[1rem] py-[3rem] gap-[3rem]'>
            <div className='flex flex-col items-center justify-center gap-[2rem] w-full'>
                ...
                <div className="flex items-center gap-[1rem]flex flex-wrap flex-col xl:flex-row lg:flex-row md:flex-row items-center justify-center gap-[1rem] p-3 w-full">
                    {branches.map((branch: BranchTypePreview) => (
                        <Link key={branch.id} href={`/rentals?page=1&branch=${branch.id}&branchName=${branch.name}`} className='relative flex flex-wrap items-end justify-start w-[350px] h-[200px] xl:h-[337.5px] lg:h-[337.5px] bg-[#1D242B] rounded-[10px] transition-transform duration-200 hover:-translate-y-1 active:translate-y-1 overflow-hidden'>

                            <img src={branch.branch_image} className="w-full h-full object-cover"/>
                            <div className="absolute inset-0 bg-gradient-to-t via-[#FAFAFAFA]/0 from-[#1D242B]/100 to-transparent" />
                            ...
                            <div className='absolute flex items-center justify-center w-full gap-2 pb-8 px-4 cursor-pointer transition-all duration-100'>
                                <div className='flex flex-col items-center gap-4'>
                                    ...
                                    <span className='text-[16px] leading-[1] text-center text-[#FAFAFA]'>{branch.address}</span>
                                </div>
                            </div>
                        </Link>
                    ))}
            ...
            <div className='flex flex-col items-center justify-center gap-[2rem] w-full'>
                ...
                <div className="flex flex-col xl:flex-row lg:flex-row md:flex-row items-center justify-center gap-[1rem] w-full p-3">
                    <div className='group relative flex items-center justify-center w-full h-[200px] xl:h-[337.5px] lg:h-[337.5px] bg-[#1D242B] rounded-[10px] overflow-hidden'>
                        <img src={bedspaceImage?.asset_url} alt="bedspace" className='absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-102 transition-all duration-200'/>
                    ...
                    <div className='group relative flex items-center justify-center w-full h-[200px] xl:h-[337.5px] lg:h-[337.5px] bg-[#1D242B] rounded-[10px] overflow-hidden'>
                        <img src={apartmentImage?.asset_url} alt="bedspace" className='absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-102 transition-all duration-200'/>
